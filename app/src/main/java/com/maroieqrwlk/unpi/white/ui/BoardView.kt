...
import com.maroieqrwlk.unpi.white.eightNeighbors
import com.maroieqrwlk.unpi.white.game.Board
import com.maroieqrwlk.unpi.white.game.GameSettings
import com.maroieqrwlk.unpi.white.game.moves.ChordMove
import com.maroieqrwlk.unpi.white.game.moves.FloodRevealMove
import com.maroieqrwlk.unpi.white.game.moves.ToggleFlagMove
import com.maroieqrwlk.unpi.white.roundUp
...
class BoardView(
    ...
    private var defaultCellSize = DEFAULT_CELL_SIZE * dp
    private var cellSize = defaultCellSize

    @Suppress("DEPRECATION")
    private val flag = resources.getDrawable(R.drawable.ic_flag)

    @Suppress("DEPRECATION")
    private val mine = resources.getDrawable(R.drawable.ic_new_bomb)
    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            Log.d("ScaleFactor", "incoming scale factor ${detector.scaleFactor}")
            currentScaleFactor *= detector.scaleFactor
            if (board!!.columns <= 9) {
                currentScaleFactor = Math.max(0.87f, Math.min(currentScaleFactor, 1.2f))
            }
            if(board!!.columns <= 16) {
                currentScaleFactor = Math.max(0.46f, Math.min(currentScaleFactor, 1.2f))
            }
            Log.d("ScaleFactor", "current scale factor $currentScaleFactor")
            cellSize = defaultCellSize * currentScaleFactor
            Log.d("ScaleFactor", "---------------------------------------")
            recalculate()
            invalidate()
            return true
        ...
        override fun onScaleBegin(detector: ScaleGestureDetector?): Boolean {
            Log.d("ScaleFactor", "scaling true")
            isScaling = true
            return super.onScaleBegin(detector)
        }

        override fun onScaleEnd(detector: ScaleGestureDetector?) {
            Log.d("ScaleFactor", "scaling false")
    ...
    companion object {
        private const val DEFAULT_CELL_SIZE = 50
